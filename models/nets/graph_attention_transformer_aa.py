...
class NodeEmbeddingNetwork(torch.nn.Module):
    ...
    def __init__(self, irreps_in, irreps_hidden, max_atom_type=21, bias=True):
        
        super().__init__()
        self.max_atom_type = max_atom_type
        self.irreps_in = o3.Irreps(irreps_in)
        self.irreps_node_embedding = o3.Irreps(irreps_hidden)
        self.atom_type_lin = LinearRS(self.irreps_in,
            self.irreps_node_embedding, bias=bias)
        
    def forward(self, aa, pos, t):
        '''
            `node_atom` is a LongTensor.
        '''
        # node_atom_onehot = torch.nn.functional.one_hot(aa, self.max_atom_type).float()
        node_feat = torch.cat([aa, t, pos.view(pos.shape[0], -1)],dim=-1)
        #print("node_feat", node_feat.shape)
        node_embedding = self.atom_type_lin(node_feat)
...
class EdgeDegreeEmbeddingNetwork(torch.nn.Module):
    ...
    def forward(self, node_input, edge_attr, edge_scalars, edge_src, edge_dst, batch):
        ...
        node_features = scatter(edge_features, edge_dst, reduce='mean', dim=0, dim_size=node_features.shape[0])
...
class EdgeFeatures(torch.nn.Module):
    ...
    def forward(self, edge_src, edge_dst):
...
class GraphAttentionTransformer(torch.nn.Module):
    def __init__(self,
        ...
        irreps_edge_attr_in='65x0e', irreps_edge_attr_hid='8x0e', max_radius=5.0,
        ...
        drop_path_rate=0.0,
        mean=None, std=None, scale=None, atomref=None, bb_only=True, self_cond=False, type='unconditional'):
        super().__init__()
        ...
        # Edge features
        self.irreps_edge_attr_in = o3.Irreps(irreps_edge_attr_in)
        self.irreps_edge_attr_hid = o3.Irreps(irreps_edge_attr_hid)
        self.irreps_edge_attr_combined = '9x0e+1x1e+1x2e'
        self.edge_featurizer = EdgeFeatures()
        self.edge_attr_in = LinearRS(self.irreps_edge_attr_in, self.irreps_edge_attr_hid)
        ...
        if bb_only:
            out_irreps = '21x0e+4x1e'
            node_irreps = '43x0e+8x1e' if self_cond else '22x0e+4x1e'
        else:
            out_irreps = '21x0e+14x1e'
            node_irreps = '43x0e+28x1e' if self_cond else '22x0e+14x1e'
        self.node_embed = NodeEmbeddingNetwork(node_irreps, self.irreps_node_embedding, 21)
        ...
        self.edge_deg_embed = EdgeDegreeEmbeddingNetwork(self.irreps_node_embedding,
            self.irreps_edge_attr, self.fc_neurons)
        
        self.blocks = torch.nn.ModuleList()
        self.build_blocks()
        
        self.norm = get_norm_layer(self.norm_layer)(self.irreps_feature)
        self.head = LinearRS(self.irreps_feature, o3.Irreps(out_irreps), rescale=_RESCALE)
    ...
    def build_blocks(self):
        for i in range(self.num_layers):
            ...
            blk = TransBlock(irreps_node_input=self.irreps_node_embedding, 
                irreps_node_attr=self.irreps_node_attr,
                irreps_edge_attr=self.irreps_edge_attr_combined,
    ...
    def forward(self, t, aa, pos, edge_index,atom_mask, batch) -> torch.Tensor:
        
        if len(t.shape) == 0: # for odeint sampling TODO: need to find better way
            t = torch.ones(pos.shape[0], 1, device=pos.device) * t
        ...
        ## TODO: does it make sense using spherical harmonics as input features for diffusion
        ca = pos[:, 1]
        edge_vec = ca.index_select(0, edge_src) - ca.index_select(0, edge_dst)
        edge_sh = o3.spherical_harmonics(l=self.irreps_edge_attr,
            x=edge_vec, normalize=True, normalization='component')

        # Takes atom type and generates embeddings of shape irreps_node_embedding
        #print("node_embedding = self.node_embed(aa, pos, t)", aa.shape,pos.shape,t.shape)

        node_embedding = self.node_embed(aa, pos, t)
       # print("node_embedding = self.node_embed(aa, pos, t)", aa.shape,pos.shape,t.shape)

        #print("node_embedding",node_embedding.shape)
        # Takes edge degrees -> embeddings of shape irreps_node_embedding
        edge_length = edge_vec.norm(dim=1)
        edge_length_embedding = self.rbf(edge_length)

        edge_features = self.edge_attr_in(self.edge_featurizer(edge_src, edge_dst))
        edge_attr = torch.cat([edge_features,edge_sh],dim=-1)

        ## TODO: degree embedding probably unnecessary for fully connected graph
        edge_degree_embedding = self.edge_deg_embed(node_embedding, edge_sh,
            edge_length_embedding, edge_src, edge_dst, batch)

        node_features = node_embedding + edge_degree_embedding

        # TODO: find what node_attr does - it seems to not do anything
        node_attr = torch.ones_like(node_features.narrow(1, 0, 1))
        

        #print("node_embedding,node_input=node_features, node_attr=node_attr,edge_src=edge_src, edge_dst=edge_dst, edge_attr=edge_attr,edge_scalars=edge_length_embedding, batch=batch")
        #print(node_embedding.shape,node_features.shape,node_attr.shape,edge_src.shape,edge_dst.shape)
        ...
        aa = outputs[:,:21] #F.softmax(outputs[:,:21],dim=-1)
        pos = outputs[:,21:].view(outputs.shape[0],-1,3)
